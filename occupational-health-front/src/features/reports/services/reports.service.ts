...
async function downloadConsolidacionReport(filters: ConsultationReportFilters): Promise<void> {
  ...
  const response = await apiClient.get('/reports/consolidacion', { params, responseType: 'blob' });
  const url = URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }));
  const link = document.createElement('a');
  link.href = url;
  link.download = `reporte-consolidacion-${new Date().toISOString().slice(0, 10)}.pdf`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  URL.revokeObjectURL(url);
