...
export class CreatePatientDto {
  ...
  @Matches(/^[VEve]-?\d{7,}$/, { message: 'La cédula debe tener al menos 7 dígitos y comenzar con V o E.' })
  ...
  @ValidateNested({ message: 'El contacto de emergencia tiene campos inválidos.' })
  ...
  @IsOptional()
  @IsIn(BLOOD_TYPES, { message: `El grupo sanguíneo debe ser uno de los siguientes: ${BLOOD_TYPES.join(', ')}.` })
  bloodType?: string;

  @IsOptional()
  @IsIn(DOMINANT_HANDS, { message: `La mano dominante debe ser una de las siguientes: ${DOMINANT_HANDS.join(', ')}.` })
  dominantHand?: string;

  @IsOptional()
  @IsBoolean({ message: 'El campo de uso de lentes debe ser verdadero o falso.' })
  usesGlasses?: boolean;
  ...
  @IsUUID('4', { each: true, message: 'Cada ID de alergia debe ser un UUID válido.' })
  @ArrayUnique({ message: 'No se permiten alergias duplicadas.' })
  allergyIds?: string[];

  @IsOptional()
  @IsArray({ message: 'Las enfermedades deben enviarse como un arreglo de IDs.' })
  @IsUUID('4', { each: true, message: 'Cada ID de enfermedad debe ser un UUID válido.' })
