...
import { consultationDisabilities, ConsultationDisability } from './consultation-disabilities.schema';
...
export class ConsultationDisabilitiesService {
  ...
  async findByConsultation(consultationId: string): Promise<ConsultationDisability[]> {
    return this.db
      .select()
  ...
  async findByPatient(patientCedula: string): Promise<{ id: string; name: string }[]> {
    return this.db
      ...
      .innerJoin(disabilities, eq(consultationDisabilities.disabilityId, disabilities.id))
      .where(eq(requests.patientId, patientCedula))
      .orderBy(disabilities.name);
  }

  async add(dto: AddConsultationDisabilityDto): Promise<ConsultationDisability> {
    ...
    if (existing) {
      throw new ConflictException('Esta discapacidad ya está registrada en la consulta.');
    }

    const [created] = await this.db
      .insert(consultationDisabilities)
      .values({ consultationId: dto.consultationId, disabilityId: dto.disabilityId })
      .returning();
  ...
  async remove(id: string): Promise<void> {
    ...
    if (!record) {
      throw new NotFoundException(`No se encontró el registro de discapacidad con ID "${id}".`);
    }

    await this.db.delete(consultationDisabilities).where(eq(consultationDisabilities.id, id));
  }
}
